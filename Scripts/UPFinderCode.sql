...
CREATE TABLE IF NOT EXISTS `UPFinder`.`have` (
  `BookingId` INT NOT NULL,
  `LibraryWorkingAreaID` INT NOT NULL,
  PRIMARY KEY (`BookingId`, `LibraryWorkingAreaID`),
  INDEX `fk_have_LibraryWorkingAreas1_idx` (`LibraryWorkingAreaID` ASC),
  ...
  CONSTRAINT `fk_have_LibraryWorkingAreas1`
    FOREIGN KEY (`LibraryWorkingAreaID`)
    REFERENCES `UPFinder`.`LibraryWorkingAreas` (`LibraryWorkingAreaID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `UPFinder`.`areDoneIn` (
  ...
  INDEX `fk_areDoneIn_Classrooms1_idx` (`ClassroomID` ASC),
  ...
  CONSTRAINT `fk_areDoneIn_Classrooms1`
    ...
    REFERENCES `UPFinder`.`Classrooms` (`ClassroomID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
